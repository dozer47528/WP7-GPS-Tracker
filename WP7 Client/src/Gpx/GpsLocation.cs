...
namespace Gps
{
    struct GpsLocation
    {
        ...
        {
            get { return (int)Decimal.Truncate((decimal)Latitude_); }
        }

        public int LatitudeMinute
        {
            get { return (int)Decimal.Truncate(60 * ((decimal)Latitude_ - Decimal.Truncate((decimal)Latitude_))); }
        }
        ...
        public int LatitudeSecond
        {
            get
            {
                double minutes = 60 * (double)((decimal)Latitude_ - Decimal.Truncate((decimal)Latitude_));
                return (int)Decimal.Truncate(60 * ((decimal)minutes - Decimal.Truncate((decimal)minutes)));
            }
        }
        ...
        public double LatitudeFractionalSecond
        {
            ...
            {
                double minutes = 60 * (double)((decimal)Latitude_ - Decimal.Truncate((decimal)Latitude_));
                return 60 * (double)((decimal)minutes - Decimal.Truncate((decimal)minutes));
            }
        }

        public int LongitudeDegree
        {
            get { return (int)Decimal.Truncate((decimal)Longitude_); }
        }

        public int LongitudeMinute
        {
            get { return (int)Decimal.Truncate(60 * ((decimal)Longitude_ - Decimal.Truncate((decimal)Longitude_))); }
        }
        ...
        public int LongitudeSecond
        {
            get
            {
                double minutes = 60 * (double)((decimal)Longitude_ - Decimal.Truncate((decimal)Longitude_));
                return (int)Decimal.Truncate(60 * ((decimal)minutes - Decimal.Truncate((decimal)minutes)));
            }
        }
        ...
        {
            double degrees = (double)Decimal.Truncate((decimal)value);
            sb.AppendFormat("F0", degrees).Append(DEGREES_SIGN).Append(' ');

            value = 60 * (value - degrees);
            double minutes = (double)Decimal.Truncate((decimal)value);
            sb.AppendFormat("F0", minutes).Append(MINUTES_SIGN).Append(' ');

            value = 60 * (value - minutes);
            double seconds = (double)Decimal.Truncate((decimal)value);
